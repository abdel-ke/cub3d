...
/*   Updated: 2020/12/26 15:18:40 by abdel-ke         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*ft_strjoin_2(t_parcing *p, char *s, char *s1, char *s2)
{
	char		*mm;
	size_t		i;
	size_t		j;
	...
	if (s && s1 && s2)
	{
		i = ft_strlen(s) + ft_strlen(s1) + ft_strlen(s2) + 1;
		mm = (char*)malloc(i * sizeof(char));
		if (mm == NULL)
			return (NULL);
		i = 0;
		while (s[i])
